...
from legged_gym.utils import rewards
...
class Bolt6_Getup(LeggedRobot):
    ...
    # ** Learn to Get up rewards ** #
    
    # def _reward_standing(self):
    #     standing = rewards.tolerance(self.base_pos[:, 2:3],
    ...
    #     horizontal_velocity = self.base_lin_vel[:, 0:2]
    #     return rewards.tolerance(horizontal_velocity, bounds=[-0.1, 0.1], margin=1.2).mean()
    ...
    def _reward_standing(self):
        ...
        error = (standing - self.cfg.rewards.standing_target)
        error = error.flatten()
        return torch.exp(-torch.square(error)/self.cfg.rewards.tracking_sigma)
    ...
    # ** prev rewards for pb rewards ** #

    def pre_physics_step(self):
    #     self.reward_orientation_prev = self._reward_orientation()
    #     self.reward_base_height_prev = self._reward_base_height()
    #     self.reward_joint_regularization_prev = self._reward_joint_regularization()
    #     self.reward_energy_prev = self._reward_energy()
    #     self.reward_action_rate_prev = self._reward_action_rate()
        
    #     self.reward_stand_still_prev = self._reward_stand_still()
    #     self.reward_no_fly_prev = self._reward_no_fly()
    #     self.reward_feet_air_time_prev = self._reward_feet_air_time()
        self.reward_standing_prev = self._reward_standing()
        self.reward_dont_move_prev = self._reward_dont_move()
        self.reward_upright_prev = self._reward_upright()
    ...
    def update_command_curriculum(self, env_ids):
        ...
        """
        #If the tracking reward is above the 80% of maximum, increase the range of commands
        if torch.mean(self.episode_sums["traking_lin_vel"][env_ids])/ self.max_episode_length > 0.70 * self.reward_scales["traking_lin_vel"]:
        ...
        if torch.mean(self.episode_sums["tracing_ang_vel"][env_ids])/self.max_episode_length > 0.50 * self.reward_scales["tracking_ang_vel"]:
    ...
    # ** pb rewards ** #
    # def _reward_orientation_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_orientation() - self.reward_orientation_prev)
    #     return delta_phi / self.dt
    
    # def _rewarrd_joint_regulation_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_joint_regularization() - self.reward_joint_regularization_prev)
    #     return delta_phi / self.dt
        
    # def _reward_base_height_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_base_height() - self.reward_base_height_prev)
    #     return delta_phi / self.dt
    
    # def _reward_energy_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_energy() - self.reward_energy_prev)
    #     return delta_phi / self.dt
    
    # def _reward_action_rate_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_action_rate() - self.reward_action_rate_prev)
    #     return delta_phi / self.dt
        
    # def _reward_stand_still_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_stand_still() - self.reward_stand_still_prev)
            
    
    # def _reward_no_fly_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_no_fly() - self.reward_no_fly_prev)
    #     return delta_phi / self.dt

    # def _reward_feet_air_time_potentialBased(self):
    #     delta_phi = ~self.reset_buf \
    #         * (self._reward_feet_air_time() - self.reward_feet_air_time_prev)
    #     return delta_phi / self.dt
    def _reward_standing_potentialBased(self):
        delta_phi = ~self.reset_buf \
            * (self._reward_standing() - self.reward_standing_prev)    
        return delta_phi / self.dt

    def _reward_dont_move_potentialBased(self):
        delta_phi = ~self.reset_buf \
            * (self._reward_dont_move() - self.reward_dont_move_prev) 
        return delta_phi / self.dt
    
    def _reward_upright_potentialBased(self):
        delta_phi = ~self.reset_buf \
            * (self._reward_upright() - self.reward_upright_prev) 
        return delta_phi / self.dt
